...
@pytest.fixture
def imputation_file_path(tmpdir):
    """Define a temporary path to store the imputations used in preprocessing."""
    return os.path.join(str(tmpdir), 'imputations.joblib')


@pytest.fixture
def regressor(imputation_file_path, sample_train_data, schema_provider):
    ...
    sample_transformed_train_data = run_pipeline(sample_train_data, schema_provider, imputation_path=imputation_file_path)
    sample_transformed_train_data[schema_provider.target] = target
    regressor = Regressor(sample_transformed_train_data, schema=schema_provider)
    return regressor


def test_fit_predict(imputation_file_path, regressor, sample_test_data, schema_provider):
    """
    ...
    sample_transformed_test_data = run_pipeline(
        sample_test_data,
        schema_provider,
        training=False,
        imputation_path=imputation_file_path
    )

    regressor.train()

    predictions = regressor.predict(sample_transformed_test_data)
